...
class UserService {
    ...
    @MainActor
    func updateUserPhotoProfile(withImageUrl imageUrl: String) async throws {
        guard let uid = Auth.auth().currentUser?.uid else { return }
        try await FirestoreContants.UsersCollection.document(uid).updateData([
            "profileImageUrl" : imageUrl
        ])
        self.currentUser?.photoProfileUrl = imageUrl
    }
}
